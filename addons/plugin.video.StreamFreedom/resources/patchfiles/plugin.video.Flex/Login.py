...
UsersXml = xbmcvfs.translatePath(os.path.join(
    'special://home/userdata/addon_data/' + _addon_id_, 'users.xml'))
#############################################################
#################### SET ADDON THEME IMAGES #################
Background_Image = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'loginbackgroundblack.png'))
# FRAMES
MaleAdult = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'man.png'))
FemaleAdult = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'women.png'))
MaleChild = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'boy.png'))
FemaleChild = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'girl.png'))
...
UserAdd = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'Adduser.png'))
UserAddS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'AdduserS.png'))
UserRemove = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'Deleteuser.png'))
UserRemoveS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'DeleteuserS.png'))
...
addontemp = xbmcvfs.translatePath(os.path.join(
    ...
    if not 'EXPIRED' in pin:
        ...
        if len(link) <= 2 or 'Pin Expired' in link:
            ...
            if registerpin == 'False':
                try:
                    requests.get(
                        'https://pinsystem.co.uk/checker.php?code=99999&plugin=Flex').text
...
def CheckUsers(self):
    with open(UsersXml) as f:
        ...
        if first == '':
            dialog.ok(
                AddonTitle, "Welcome To Flex, No Users Have Been Setup Yet, Please Create Your First User, Click Ok To Proceed")
            string = ''
            keyboard = xbmc.Keyboard(
                string, '[COLOR white][B]What Is The Users Name?[/B][/COLOR]')
            keyboard.doModal()
            if keyboard.isConfirmed():
                string = keyboard.getText()
                if len(string) >= 1:
                ...
                dialog.ok(
                    AddonTitle, "Sorry You Need To Setup At Least One User")
                quit()
            MORF = xbmcgui.Dialog().yesno(AddonTitle, 'Is %s Male Or Female?' %
                                          username, yeslabel='Male', nolabel='Female')
            if MORF:
                sex = 'MALE'
            else:
                sex = 'WOMEN'
            AGE = xbmcgui.Dialog().yesno(AddonTitle, 'Is %s An Adult Or Child?' %
                                         username, yeslabel='Adult', nolabel='Child')
            if AGE:
                prop = 'ADULT'
            else:
                prop = 'CHILD'
...
def DeleteUser(self):
    ...
    dialog.notification(
        AddonTitle, '[COLOR red][B]User %s Removed[/B][/COLOR]' % names[select], icon, 5000, False)
    MainWindow()
    self.close()
...
def AddUser(self):
    if Person4 == '':
        ...
        dialog.notification(
            AddonTitle, '[COLOR red][B]Sorry You Can Only Have 4 Users[/B][/COLOR]', icon, 5000, False)
...
class Main(pyxbmct.AddonFullWindow):
    ...
    def __init__(self, title='Flex'):
        ...
        NameTitle1 = pyxbmct.Label(
            Person1, textColor='0xFFFFFFFF', font='font18', alignment=pyxbmct.ALIGN_CENTER)
        NameTitle2 = pyxbmct.Label(
            Person2, textColor='0xFFFFFFFF', font='font18', alignment=pyxbmct.ALIGN_CENTER)
        NameTitle3 = pyxbmct.Label(
            Person3, textColor='0xFFFFFFFF', font='font18', alignment=pyxbmct.ALIGN_CENTER)
        NameTitle4 = pyxbmct.Label(
            Person4, textColor='0xFFFFFFFF', font='font18', alignment=pyxbmct.ALIGN_CENTER)
    ...
    def set_active_controls(self):
        ...
        self.button5 = pyxbmct.Button(
            '',   focusTexture=UserAddS,   noFocusTexture=UserAdd)
        self.placeControl(self.button5, 82, 22, 11, 6)
        self.button6 = pyxbmct.Button(
            '',   focusTexture=UserRemoveS,   noFocusTexture=UserRemove)
