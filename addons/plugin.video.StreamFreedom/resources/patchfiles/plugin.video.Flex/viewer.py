...
MoviesXml = xbmcvfs.translatePath(os.path.join(
    'special://home/userdata/addon_data/' + _addon_id_, '%smovies.xml'))
ShowsXml = xbmcvfs.translatePath(os.path.join(
    'special://home/userdata/addon_data/' + _addon_id_, '%sshows.xml'))
#############################################################
...
Background_Image = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'Viewerblack.png'))
AddMovie = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'addmovie.png'))
AddMovieS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'addmovieS.png'))
AddShow = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'addshow.png'))
AddShowS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'addshowS.png'))
DeleteMovie = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'deletemovie.png'))
DeleteMovieS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'deletemovieS.png'))
DeleteShow = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'deleteshow.png'))
DeleteShowS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'deleteshowS.png'))
ViewSeasons = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'viewseasons.png'))
ViewSeasonsS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'viewseasonsS.png'))
FindLinks = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'findlinks.png'))
FindLinksS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'findlinksS.png'))
Trailer = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'trailer.png'))
TrailerS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'trailerS.png'))
Back = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'back.png'))
BackS = xbmcvfs.translatePath(os.path.join(
    'special://home/addons/' + _addon_id_ + _images_, 'backS.png'))
Listbg = xbmcvfs.translatePath(os.path.join(
...
def MainWindow(type, movieid, user):
    ...
    if 'Movie' in source:
        MovieID = movieid
        url = ('https://api.themoviedb.org/3/movie/%s?api_key=%s&append_to_response=videos' %
               (movieid, TMDBAPI))
        ...
        AddTitle = str(title)
        title = '[B]'+str(title)+'[/B]'
        MovieTitle = pyxbmct.Label(
            title, textColor='0xFFFFFFFF', font='font60', alignment=pyxbmct.ALIGN_LEFT)
        releasedate = link['release_date']
        MovieYear = releasedate.split('-')[0]
        dateformat = 'Released : ' + str(releasedate)
        Release = pyxbmct.Label(
            dateformat, textColor='0xFFFFFFFF', font='font24', alignment=pyxbmct.ALIGN_LEFT)
        desc = link['overview']
        ...
        MovieRating = pyxbmct.Label(
            rating, textColor='0xFFFFFFFF', font='font24', alignment=pyxbmct.ALIGN_LEFT)
        runtime = link['runtime']
        runtime = 'Movie Length : ' + str(runtime) + ' Mins'
        MovieLength = pyxbmct.Label(
            runtime, textColor='0xFFFFFFFF', font='font24', alignment=pyxbmct.ALIGN_LEFT)
        window = Main('Viewer')
        window.doModal()
        del window
    else:
        MovieID = movieid
        url = ('https://api.themoviedb.org/3/tv/%s?api_key=%s&append_to_response=videos' %
               (movieid, TMDBAPI))
        link = requests.get(url).json()
        poster = link['poster_path']
        Poster = 'https://image.tmdb.org/t/p/original' + poster
...
def PlayTrailer(self, id, desc):
    ...
    if 'No Trailer' in id:
        dialog.notification(
            AddonTitle, '[COLOR red][B]Sorry No Trailer Available For This Media[/B][/COLOR]', icon, 5000, False)
        quit()
    dialog.notification(
        AddonTitle, '[COLOR red][B]Finding Trailer Source Now[/B][/COLOR]', icon, 5000, False)
    link = requests.get(url).json()
    ...
    select = dialog.select(
        '[B][COLOR red]F[COLOR white]ound these trailers[/COLOR][/B]', trailertitle)
    if select < 0:
        quit()
...
def AddMovieToList(self, AddTitle, AddPoster, MovieID, IMDBNO, User):
    with open(MoviesXml % User, 'a') as f:
        ...
        f.close()
        dialog.notification(
            AddonTitle, '[COLOR red][B]Movie Added To Your Collection[/B][/COLOR]', icon, 5000, False)
...
def AddShowToList(self, AddTitle, AddPoster, MovieID, User):
    with open(ShowsXml % User, 'a') as f:
        ...
        f.close()
        dialog.notification(
            AddonTitle, '[COLOR red][B]Show Added To Your Collection[/B][/COLOR]', icon, 5000, False)
...
def FindSources(self, name, year, IMDBNO, desc):
    ...
    Item_Title.append(
        '[B][COLOR deepskyblue]SOURCES FOR % s[/B][/COLOR]' % name)
    self.List.addItem(
        '[B][COLOR deepskyblue]SOURCES FOR % s[/B][/COLOR]' % name)
    Item_Link.append('')
    ...
    for data in link['streams']:
        ...
        self.List.addItem(
            '[B][COLOR deepskyblue]%s | %s[/B][/COLOR]' % (quality, updated_title))
    ...
    with open(MoviesXml % User, "w") as f:
        for line in lines:
            if MovieID not in line:
                f.write(line)
    dialog.notification(
        AddonTitle, '[COLOR red][B]Movie Removed From Your Collection[/B][/COLOR]', icon, 5000, False)
    ...
    with open(ShowsXml % User, "w") as f:
        for line in lines:
            if MovieID not in line:
                f.write(line)
    dialog.notification(
        AddonTitle, '[COLOR red][B]Show Removed From Your Collection[/B][/COLOR]', icon, 5000, False)
    ReturnHome(self, User)
...
def Player(name, url, iconimage, desc):
    try:
        ...
        dialog.notification(
            AddonTitle, "[B][COLOR yellow]ERROR %s[/B][/COLOR]" % c, Addonicon, 5000)
...
class Main(pyxbmct.AddonFullWindow):
    ...
    def __init__(self, title='Flex'):
        ...
        self.connect(self.button2, lambda: DeleteMovieFromList(
            self, removeme, MovieID, User))
        self.connect(self.button1, lambda: AddMovieToList(
            self, AddTitle, AddPoster, MovieID, IMDBNO, User))
        self.connect(self.button3, lambda: PlayTrailer(self, trailerkey, desc))
        self.connect(self.button4, lambda: FindSources(
            self, AddTitle, MovieYear, IMDBNO, desc))
        self.connect(self.List, lambda: List_Selected(self))
        if 'TV' in source:
            self.connect(self.button6, lambda: AddShowToList(
                self, AddTitle, AddPoster, MovieID, User))
            self.connect(self.button7, lambda: DeleteShowFromList(
                self, removeme, MovieID, User))
            self.connect(self.button8, lambda: ShowSeasons(
                self, removeme, MovieID))
        ShowText(self, desc)
    ...
    def set_active_controls(self):
        ...
        if 'TV' in source:
            self.button6 = pyxbmct.Button(
                '',   focusTexture=AddShowS,   noFocusTexture=AddShow)
            self.placeControl(self.button6, 95, 16,  7, 5)
            self.button7 = pyxbmct.Button(
                '',   focusTexture=DeleteShowS,   noFocusTexture=DeleteShow)
            self.placeControl(self.button7, 95, 22,  7, 5)
            self.button8 = pyxbmct.Button(
                '',   focusTexture=ViewSeasonsS,   noFocusTexture=ViewSeasons)
