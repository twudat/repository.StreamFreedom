...
specific_icon = translatePath(os.path.join(
    'special://home/addons/script.xxxodus.artwork/resources/art/', '%s/icon.png'))
specific_fanart = translatePath(os.path.join(
    'special://home/addons/script.xxxodus.artwork/resources/art/', '%s/fanart.jpg'))
...
def mainMenu():
    ...
    art = translatePath(os.path.join(
        'special://home/addons/script.xxxodus.artwork/resources/art/', 'main/%s.png'))
    CheckParent()
    dirlst = []
    c = []
    c += [
        (kodi.giveColor('Welcome to XXX-O-DUS Version %s' % kodi.get_version(), 'blue', True), translatePath(os.path.join(kodi.addonfolder,
         'resources/files/information.txt')), 17, 'icon', 'Original Code by EchoCoder, Please Report All issues to @Nemzzy668', False),
        (kodi.giveColor('Official Version Now Maintained By [COLOR yellow]@Nemzzy668[/COLOR]', 'blue', True), translatePath(os.path.join(
            kodi.addonfolder, 'resources/files/information.txt')), 17, 'icon', 'Please Report any issues to @Nemzzy668 On Twitter', False),
        ('[COLOR yellow]View Changelog[/COLOR]', translatePath(os.path.join(kodi.addonfolder,
         'changelog.txt')), 17, 'changelog', 'View XXX-O-DUS Changelog.', False),
        ...
        ('[COLOR pink]Virtual Reality', None,
         42, 'vr', 'XXX Virtual Reality', True),
        ...
        (kodi.giveColor('Report Issues on Twitter @Nemzzy668', 'violet', True), translatePath(os.path.join(kodi.addonfolder, 'resources/files/information.txt')),
         17, 'report', 'All issues must be reported to me at twitter, I can\'t fix them, if I don\'t know about them!', False),
    ]
...
def cams():
    ...
    if cam_sources:
        ...
        for i in sorted(cam_sources):
            dirlst.append({'name': kodi.giveColor(i[0], 'white'), 'url': None, 'mode': i[1],
                          'icon': specific_icon % i[2], 'fanart': specific_fanart % i[2], 'folder': True})
...
@utils.url_dispatcher.register('99')
def livestreams():
...
def installcum():
    xbmc.executebuiltin("Container.Update(plugin://plugin.video.cwm)")
...
def movies():
    ...
    if movies_sources:
        ...
        for i in sorted(movies_sources):
            dirlst.append({'name': kodi.giveColor(i[0], 'white'), 'url': None, 'mode': i[1], 'icon': specific_icon % i[0].lower(
...
def versioncheck():
    checkxxx = translatePath(os.path.join(
        'special://home/addons/' + addon_id, 'addon.xml'))
    checkscraper = translatePath(os.path.join(
        'special://home/addons/' + scraper_id, 'addon.xml'))
    checkartwork = translatePath(os.path.join(
        'special://home/addons/' + artwork_id, 'addon.xml'))
    ...
    checkcurrentaddon = re.findall(
        '<addon\s+id="plugin.video.xxx-o-dus".+?version="(.*?)"', currentversions, flags=re.DOTALL)[0]
    checkcurrentscraper = re.findall(
        '<addon\s+id="script.xxxodus.scrapers".+?version="(.*?)"', currentversions, flags=re.DOTALL)[0]
    checkcurrentartwork = re.findall(
        '<addon\s+id="script.xxxodus.artwork".+?version="(.*?)"', currentversions, flags=re.DOTALL)[0]
    ...
    if d == checkcurrentscraper:
        string = string + \
            ('[COLOR lime]Scraper MODULE Current : %s | Your Version : %s | All Up To Date[/COLOR]\n' %
             (checkcurrentscraper, d))
    elif d < checkcurrentscraper:
        string = string + \
            ('[COLOR orange]Scraper MODULE : %s | Your Version : %s | Module Out Of Date[/COLOR]\n' %
             (checkcurrentscraper, d))
    elif d > checkcurrentscraper:
        string = string + \
            ('[COLOR red][B]Scraper MODULE : %s | Your Version : %s | Malicous Version Detected - Recommended You Un-install[/B][/COLOR]\n' %
             (checkcurrentscraper, d))

    if f == checkcurrentartwork:
        string = string + \
            ('[COLOR lime]Artwork MODULE Current : %s | Your Version : %s | All Up To Date[/COLOR]\n' %
             (checkcurrentartwork, f))
    elif f < checkcurrentartwork:
        string = string + \
            ('[COLOR orange]Artwork MODULE : %s | Your Version : %s | Module Out Of Date[/COLOR]\n' %
             (checkcurrentartwork, f))
    elif f > checkcurrentartwork:
        string = string + \
            ('[COLOR red][B]Artwork MODULE : %s | Your Version : %s | Malicous Version Detected - Recommended You Un-install[/B][/COLOR]\n' %
             (checkcurrentartwork, f))
    ...
    if not 'EXPIRED' in pin:
        ...
        if len(link) <= 5 or 'Pin Expired' in link:
            ...
            if registerpin == 'False':
                try:
                    requests.get(
                        'https://pinsystem.co.uk/checker.php?code=99999&plugin=XXXODUS').content
